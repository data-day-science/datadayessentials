...
class TestExecutionEnvironmentManager(unittest.TestCase):
    ...
    @patch("platform.system", return_value="Linux")
    def test_asser_error_no_environment_variable(
        self, mock_platform_system, mock_os_getenv
    ):
        self.assertRaises(
            EnvironmentError, ExecutionEnvironmentManager.get_execution_environment
        )
