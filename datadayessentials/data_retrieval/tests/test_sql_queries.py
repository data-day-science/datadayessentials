...
class TestQueryFormatter(unittest.TestCase):
    ...
    @patch(
        "builtins.open",
        new_callable=MockOpen(
            read_data="SELECT * FROM test_table WHERE column1 = '{param1}' AND column2 = '{param2}'"
        ),
    )
    ...
    def test_format_query(self, mock_open):
        ...
        self.assertEqual(
            formatted_query,
            "SELECT * FROM test_table WHERE column1 = 'value1' AND column2 = 'value2'",
        )
