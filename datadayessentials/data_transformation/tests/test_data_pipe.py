...
from .._data_pipe import (
    DataFramePipe,
    _get_nth_columns_at_increasing_indexes,
    _append_target_in_each_list_without_it,
    _clean_up_processed_data,
...
class TestRunPipelineWithMultiThreading(unittest.TestCase):
    ...
    def test_append_target_in_each_list_without_it(self):
        ...
        test_target = "A"
        updated_feature_combinations = _append_target_in_each_list_without_it(
            test_feature_combinations, test_target
        )
        assert updated_feature_combinations == [
            ["A", "D", "G", "J"],
            ["B", "E", "H", "A"],
            ["C", "F", "I", "A"],
        ]
